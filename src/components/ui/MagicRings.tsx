...
const parseColor = (c: string) => {
  if (c.startsWith('#')) {
    ...
    if (c.length === 5) return c.slice(0, 4)
  }
  return c
}
...
export function MagicRings({
  ...
  useEffect(() => {
    ...
    mount.addEventListener('mouseenter', () => { isHoveredRef.current = true })
    mount.addEventListener('mouseleave', () => { isHoveredRef.current = false; mouseRef.current[0] = 0; mouseRef.current[1] = 0 })
    mount.addEventListener('click', () => { burstRef.current = 1 })
    let frameId: number
    const animate = (t: number) => {
      ...
      burstRef.current *= 0.95; if (burstRef.current < 0.001) burstRef.current = 0
      uniforms.uTime.value = t * 0.001 * p.speed
      uniforms.uAttenuation.value = p.attenuation; uniforms.uColor.value.set(parseColor(p.color)); uniforms.uColorTwo.value.set(parseColor(p.colorTwo))
      ...
      uniforms.uHoverAmount.value = hoverAmountRef.current; uniforms.uHoverScale.value = p.hoverScale
      uniforms.uParallax.value = p.parallax; uniforms.uBurst.value = p.clickBurst ? burstRef.current : 0
      renderer.render(scene, camera)
    }
    frameId = requestAnimationFrame(animate)
    return () => {
      cancelAnimationFrame(frameId); window.removeEventListener('resize', resize); ro.disconnect()
